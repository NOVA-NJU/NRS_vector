...
1. **首次启动时间**: 首次运行会下载嵌入模型（约 150MB），需要一定时间和网络连接
2. **数据持久化**: 所有文档和向量存储在 `./chroma_db/` 目录，请确保：
   - 目录有写权限
   - 有足够的磁盘空间（每 10k 文档约需 20-50MB）
   - 定期备份重要数据
3. **模型选择**: `bge-small-zh-v1.5` 针对中文优化，如需处理英文可更换为 `all-MiniLM-L6-v2` 等模型
4. **性能考量**: 
   - HNSW 索引在大规模数据（10k+ 文档）下表现优异
   - 小规模数据（< 1k）时，暴力搜索可能更快
   - 查询性能与 top_k 值成正比
5. **分块配置**: 
